...
class OmniLayerNorm(nn.Module):
    def __init__(self, ori_layer_norm) -> None:
        ...
        """
        对opt-125m, eps=1e-5, normalized_shape=(768,)
        """
    ...
    def forward(self, x):
        if self.use_temporary_parameter:
            ...
            weight = self.weight
            bias = self.bias
        if hasattr(self, "reorder_index"):                              # ltl新加分支
            if x.ndim == 3:                                             # reorder along columns. 这里可能带来访存不方便的问题
                x = torch.index_select(x, 2, self.reorder_index)
            elif x.ndim == 2:                                           

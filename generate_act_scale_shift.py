...
def build_model_and_tokenizer(model_name, model_cache_dir):
    kwargs = {"torch_dtype": torch.float16, "device_map": "auto"}
    model = OPTForCausalLM.from_pretrained(
                model_name, cache_dir=model_cache_dir, torch_dtype=torch.float32, device_map="auto"
            )                                                   # 我在生成opt-125m模型的scales时，要把torch_dtype改成torch.float32才能跑。本来是torch.float16
    tokenizer = AutoTokenizer.from_pretrained(
        model_name, cache_dir=model_cache_dir, use_fast=False
    )
...
def parse_args():
    ...
    parser.add_argument('--model', type=str,
                        default='../LLMs/opt/125m', help='model name')
    parser.add_argument('--scales-output-path', type=str, default='./ltl_scales/',
                        help='where to save the act scales')
    parser.add_argument('--shifts-output-path', type=str, default='./ltl_shifts/',
...
def main():
    ...
    model_cache_dir = "../LLMs/opt/125m"
    # model, tokenizer = build_model_and_tokenizer(args.model)
    # model, tokenizer = build_model_and_tokenizer(model_name, model_cache_dir)
    lm = torch.load("../LLMs/opt/125m/torch_model.pth")
    model = lm.model
    ...
    # seqlen=args.seq_len,
    # )
    dataloader = torch.load("./cache/dataloader_opt_wikitext2_128.cache")

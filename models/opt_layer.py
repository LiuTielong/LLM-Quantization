...
class QuantOPTAttention(nn.Module):
    ...
    def __init__(
        ...
        self.head_dim = embed_dim // num_heads                                                                          # 64维/头
...
class QuantOPTDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        residual = hidden_states                                                                                        # [1,2048,768]
    ...
    def smooth_and_quant_inplace(self):
        # return
        if self.let:
            for name, module in self.named_parameters():                                                                # 把所有的参数的绝对值都设置>=0.01
    ...
    def smooth_and_quant_temporary(self):
        if self.let:
            ...
            smooth_fc_fc_temporary(self.self_attn.v_proj,self.self_attn.out_proj,
                                self.out_smooth_scale, self.out_smooth_shift)                                           # v_proj和out_proj之间还能做一个smooth我是真的没想到
            smooth_q_k_temporary(self.self_attn.q_proj, self.self_attn.k_proj,
                                self.qkt_smooth_scale)                                                                  # q_proj和k_proj之间的转换也很nb
            self.fc2.temp_weight = self.fc2.weight                                                                      # 竟然没有对fc2做smooth
        else:
            for name, module in self.named_modules():
                if isinstance(module, QuantLinear):
